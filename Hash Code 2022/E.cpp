...
vector<Coder*>::iterator findCoder(vector<Coder*> &available, int req, string skillName, unordered_set<string> &taken) {
    auto coder = lower_bound(all(available), req, [&](Coder *a, int target) {
        return a->skills[skillName] < target;
    });
    while(coder != available.end() && taken.find((*coder)->name) != taken.end()) {
        coder++;
    }
    return coder;
}
...
vector<pair<Project*,vector<Coder*>>> process() {
    ...
    for(auto project: projects) {
        ...
        for(auto role: project->roles) {
            ...
            auto coder = findCoder(available, req-1, skillName, taken);
            ...
            if((*coder)->skills[skillName] == req-1) {
                ...
                if(!mentee) {
                    coder = findCoder(available, req, skillName, taken);
                    if(coder == available.end()) {
                        allRolesDone = false;
                        break;
                    }
